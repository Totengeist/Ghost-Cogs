...
class AnimalFacts(commands.Cog):
    ...
    async def _print_fact(self, ctx, fact):
        ...
        await(ctx.send(fact["fact"] + " ||<@&1242549160612466799>||", suppress_embeds=True, allowed_mentions=discord.AllowedMentions(roles=True)))

    async def _print_feature(self, ctx, feature):
        await(ctx.send(f'## {feature["animal"]} (*{feature["latin"]}*)'))
        await(ctx.send(feature["image"]))
        await(ctx.send("\n\n".join(feature["feature"]) + " ||<@&1242549202869817405>||", suppress_embeds=True, allowed_mentions=discord.AllowedMentions(roles=True)))
